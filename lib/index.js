...
app.use(require('${winPath(require.resolve("dva-immer"))}')());
  ...
  function getActionContent() {
    const routeModels = isDev || !shouldImportDynamic ? [] : getModelsWithRoutes(api.routes, api);
    const globalModels = getGlobalModels(api, isDev ? false : shouldImportDynamic);

    const _exclude$reduce = exclude(globalModels, (0, _umiUtils().optsToArray)(opts.exclude)).reduce((target, path) => {
      ...
      globalActions: []
          ...
          globalActions = _exclude$reduce.globalActions;

    const _exclude$reduce2 = exclude((0, _lodash.default)(routeModels.concat(globalModels)), (0, _umiUtils().optsToArray)(opts.exclude)).reduce((target, path) => {
      const namespace = (0, _path().basename)(path, (0, _path().extname)(path));
      ...
      actions: []
          ...
          actions = _exclude$reduce2.actions;
  ...
  function getGlobalStoreStateType() {
    ...
    const _exclude$reduce3 = exclude(getGlobalModels(api, false), (0, _umiUtils().optsToArray)(opts.exclude)).reduce((target, path) => {
      const namespace = (0, _path().basename)(path, (0, _path().extname)(path));
      ...
      state: []
          ...
          imports = _exclude$reduce3.imports,
          state = _exclude$reduce3.state;
...
...((require('${dvaJS}').config || (() => ({})))()),
  ...
  function generateInitAction() {
    ...
    const _getActionContent = getActionContent(),
          _getActionContent2 = _slicedToArray(_getActionContent, 3),
          importContent = _getActionContent2[0],
          globalActionContent = _getActionContent2[1],
          actionContent = _getActionContent2[2];

    tplContent = tplContent.replace("<%= ImportActions %>", importContent).replace("<%= RegisterGlobalActions %>", globalActionContent).replace("<%= RegisterActions %>", actionContent);
...
models: () => [
  ${models.map(model => `import(${opts.dynamicImport.webpackChunkName ? `/* webpackChunkName: '${(0, _umiUtils().chunkName)(paths.cwd, model)}' */` : ""}'${model}').then(m => { const { getModel, modelsContainer } = require('dva-model-enhance'); const targetModel = getModel(m.default); const model = { namespace: '${(0, _path().basename)(model, (0, _path().extname)(model))}', ...(targetModel || m.default)}; targetModel && modelsContainer.set(model.namespace, new m.default()); return model;})`).join(",\r\n  ")}
],
      `.trim());
      }
